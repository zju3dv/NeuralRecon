...
# params
project_path = '/data/xieyiming/neufu_demo/2020-11-22T00-51-21'
data_source = 'ARKit'
window_size = 9
min_angle = 15
min_distance = 0.1
...
video_path = os.path.join(project_path, 'Frames.m4v')
...
sync_intrinsics_and_poses(os.path.join(project_path, 'Frames.txt'), os.path.join(project_path, 'ARposes.txt'),
                          os.path.join(project_path, 'SyncedPoses.txt'))

path_dict = path_parser(project_path, data_source=data_source)
cam_intrinsic_dict = load_camera_intrinsic(
    path_dict['cam_intrinsic'], data_source=data_source)

for k, v in tqdm(cam_intrinsic_dict.items(), decs='processing camera intrinsics...'):
...
if not os.path.exists(os.path.join(project_path, 'poses')):
    os.mkdir(os.path.join(project_path, 'poses'))
for k, v in tqdm(cam_pose_dict.items(), desc='saving camera extrinsics...'):
    np.savetxt(os.path.join(project_path, 'poses', '{}.txt'.format(k)), v, delimiter=' ')

if not os.path.exists(os.path.join(project_path, 'intrinsics')):
    os.mkdir(os.path.join(project_path, 'intrinsics'))
for k, v in tqdm(cam_intrinsic_dict.items(), desc='saving camera intrinsics...'):
    np.savetxt(os.path.join(project_path, 'intrinsics', '{}.txt'.format(k)), v['K'], delimiter=' ')
...
with open(os.path.join(project_path, 'fragments.pkl'), 'wb') as f:
